...
{
    public class AvSimDllCommSocket_AndroidSerial : IAvSimDllComm
    {
        private AndroidJavaClass JAVASerialCommunicationClass;
        private AndroidJavaObject JAVASerialCommunicationObject;

        private AndroidJavaObject g_serialfd;
        ...
        private bool InitSerial()
        {
            if (g_serialfd != null)
            {
                Console.WriteLine("WARNING: InitSerial(): already opened serial");
                ComClose();
            }

            JAVASerialCommunicationClass = new AndroidJavaClass("com.PNI.JAVASerialCommunication");
            JAVASerialCommunicationObject = JAVASerialCommunicationClass.CallStatic<AndroidJavaObject>("getInstance");
            ...
            CLOSE_RET:
            if (g_serialfd != null)
                JAVASerialCommunicationObject.Call("close", g_serialfd);
        ...
        public bool ComOpen(string remoteIp, int nPort)
        {
            ...
            try
            {
                g_serialfd = JAVASerialCommunicationObject.Call<AndroidJavaObject>("createSerial", remoteIp, nPort);
            }
...
CLOSE_RET:
        ...
        {
            return (g_serialfd != null) && g_connected;
        }
        ...
        public int ComRead(byte[] msg, int len)
        {
            ...
            if (!g_connected)
                return -1;
            byte[] readData = JAVASerialCommunicationObject.Call<byte[]>("ReadByte", g_serialfd, len);
        ...
        public int ComRead(byte[] aMessage, int nLength, int nRecvPacketLength)
        {
            ...
            byte[] recievedData = JAVASerialCommunicationObject.Call<byte[]>("ReadByte", g_serialfd, nLength);
        ...
        public int  ComWrite(byte[] msg, int len)
        {
            ...
            JAVASerialCommunicationObject.Call("WriteByte", g_serialfd, msg, len);
